...
This package provides the full Boost ut code, but when
installed via xpm the content is filtered and only the
`inclue` and `cmake` folders are used.
...
The project includes many class templates; see the documentation for details.
...
[Actions](https://github.com/xpack-3rd-party/boost-ut-xpack/actions/)
on each push.
